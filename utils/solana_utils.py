...
class Solana_Utils():
    ...
    def set_spl_client(solana_client: Client, token_address_publickey: Pubkey, program_id_publickey: Pubkey,
                       source_main_wallet_keypair: Keypair):
        spl_client = Token(conn=solana_client, pubkey=token_address_publickey, program_id=program_id_publickey,
    ...
    def get_token_wallet_address_from_main_wallet_address(spl_client: Token, main_wallet_address: Pubkey):
        try:
            ...
            spl_client.get_accounts_by_owner(owner=main_wallet_address, commitment=None, encoding='base64').value[
                0].pubkey
        except:
            token_wallet_address_public_key = spl_client.create_associated_token_account(owner=main_wallet_address,
    ...
    def approve_spl_token_transaction(spl_client: Token, source_token_address_publickey: Pubkey,
                                      delegate_token_address_publickey: Pubkey, owner_token_address_publickey: Pubkey,
                                      amount: float):
        amount = int(amount * 1000000000)
        transaction_signature = spl_client.approve(source_token_address_publickey, delegate_token_address_publickey,
                                                   owner_token_address_publickey, amount, multi_signers=None, opts=None,
                                                   recent_blockhash=None).value
        return transaction_signature

    def send_spl_token(spl_client: Token, source_token_address_publickey: Pubkey,
                       destination_token_address_publickey: Pubkey, source_main_wallet_keypair: Keypair, amount: float):
        transaction_signature = spl_client.transfer(source=source_token_address_publickey,
                                                    dest=destination_token_address_publickey,
                                                    owner=source_main_wallet_keypair,
                                                    amount=int(float(amount) * 1000000000), multi_signers=None,
                                                    opts=None, recent_blockhash=None).value
        return str(transaction_signature)

    def send_solana(solana_client: Client, sender_main_wallet_public_key: Pubkey,
                    destination_main_wallet_public_key: Pubkey, source_main_wallet_keypair: Keypair, amount: float):
        txn = Transaction().add(transfer(TransferParams(
            from_pubkey=sender_main_wallet_public_key, to_pubkey=destination_main_wallet_public_key,
    ...
    def check_solana_transaction(solana_client: Client, transaction_signature: Signature):
        transaction_complete = solana_client.get_transaction(tx_sig=transaction_signature, encoding='json',
                                                             commitment=None, max_supported_transaction_version=None)
    ...
    def check_token_transaction(solana_client: Client, transaction_signature: Signature):
        transaction_complete = solana_client.get_transaction(tx_sig=transaction_signature, encoding='json',
                                                             commitment=None, max_supported_transaction_version=None)
        prior_balance = transaction_complete.value.transaction.meta.post_token_balances[
            0].ui_token_amount.ui_amount_string
        initial_balance = transaction_complete.value.transaction.meta.pre_token_balances[
            0].ui_token_amount.ui_amount_string
        if prior_balance == initial_balance:
            return False
        else:
    ...
    def check_solana_transaction_direct(solana_client: Client, transaction_signature: Signature):
        transaction_status = \
        solana_client.confirm_transaction(tx_sig=transaction_signature, commitment=None, sleep_seconds=0.5,
                                          last_valid_block_height=None).value[0].confirmation_status
        return transaction_status

    def get_transaction_signature_list(solana_client: Client, owner_account: Pubkey):
        signatures = solana_client.get_signatures_for_address(owner_account, before=None, until=None, limit=None,
                                                              commitment=None).value
    ...
    def get_transaction_details_from_signature(solana_client: Client, transaction_signature: Signature):
        transaction_complete = solana_client.get_transaction(tx_sig=transaction_signature, encoding='json',
                                                             commitment=None, max_supported_transaction_version=0)
        return transaction_complete
